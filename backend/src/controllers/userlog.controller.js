...
export const getAllLogs = asyncHandler(async (req, res) => {
	...
	if (!data) {
		return errorHandler("No data found.", 400, USER_LOG);
	}
...
export const getUserLogByType = asyncHandler(async (req, res) => {
	...
	if (!type) {
		return errorHandler("Query not found.", 409, USER_LOG);
	}

	const logsData = await userLogService.fetchLogByType(type);

	if (!logsData) {
		return errorHandler(`No type ${type} data found.`, 409, USER_LOG);
	}
...
export const getUserLogsById = asyncHandler(async (req, res) => {
	...
	if (!logsData) {
		return errorHandler("ID not found or invalid", 409, USER_LOG);
	}
...
export const deleteUserLogById = asyncHandler(async (req, res) => {
	...
	if (!user) {
		return errorHandler("No data found or you don't have permission.", 409, USER_LOG);
	}
